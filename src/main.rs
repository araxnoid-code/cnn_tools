        ...
        .map(|x| {
            if (x % 3) == 0 {
                (
                    ...
                    .unwrap(),
                    ArrayD::<f32>::from_shape_vec(vec![1, 3], vec![1., 0., 0.]).unwrap(),
                )
            } else if (x % 2) == 0 {
                (
                    ArrayD::from_shape_vec(
                        vec![3, 32, 32],
                        (0..3 * 32 * 32).map(|x| x as f32 / 2048.).collect(),
                    )
                    .unwrap(),
                    ArrayD::<f32>::from_shape_vec(vec![1, 3], vec![0., 1., 0.]).unwrap(),
                )
            } else {
                (
                    ArrayD::from_shape_vec(
                        vec![3, 32, 32],
                        (0..3 * 32 * 32).map(|x| x as f32 / 3072.).collect(),
                    )
                    .unwrap(),
                    ArrayD::<f32>::from_shape_vec(vec![1, 3], vec![0., 0., 1.]).unwrap(),
                )
            }
        })
        .collect::<Vec<(ArrayD<f32>, ArrayD<f32>)>>();

    // model
    let mut conv2d_1 = Conv2DNonBatch::new(3, 24, 3);
    let max_pooling_1 = MaxPooling2DNonBatch::new(2, 2);

    let mut conv2d_2 = Conv2DNonBatch::new(24, 16, 3);
    let max_pooling_2 = MaxPooling2DNonBatch::new(2, 2);

    let mut linear_1 = LinaerNonBatch::new(576, 256);
    let mut linear_2 = LinaerNonBatch::new(256, 3);
